...
# This Solution must be modified for more than 1 amount!!!

def convert_to_separate_tuples(weight: int, items: list[tuple[int, int] | tuple[int, int, int]]):
    capacity = weight
    profits, weights = list(), list()
    for item in items:
...
def knapsack(weight: int, items: list[tuple[int, int] | tuple[int, int, int]]) -> int:
    profits, weights, capacity = convert_to_separate_tuples(weight, items)
