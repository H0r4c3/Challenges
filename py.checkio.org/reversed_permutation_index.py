...
You are given the length of the permutation (an integer) and the permutation index (an integer starting from 1). 
The task is to calculate one of the permutations of the consecutive integers (starting from 0) and return it (an iterable of integers).
'''
...
import itertools
def reversed_permutation_index_(length: int, index: int) -> Iterable[int]:
    numbers = tuple(range(length))
    sorted_perm = sorted(itertools.permutations(numbers))
    
    print(sorted_perm[index-1])
    return sorted_perm[index-1]
...
def reversed_permutation_index(length: int, index: int) -> Iterable[int]:
    ...
    while length > 0:
        length -= 1
        x, index = divmod(index, math.factorial(length))
        perm.append(numbers[x])
