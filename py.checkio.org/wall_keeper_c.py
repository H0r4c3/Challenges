...
def wall_keeper(on_panels):
    ...
    # set all cells in on_panels to ON
    ...
    # click on 'down' neighbors for all ON in a row, one by one    
    for val in range(1, 21):
            cell = Cell.get_cell_by_value(val)
            if cell.light == True:
                print(f'The cell {cell.down} will be clicked')
                cell.down.click()
                result.append(cell.down.value)
                Cell.print_grid()
    
    print(f'The final grid is: ')
    Cell.print_grid()
                
    # verify that all cells in final row are OFF
    last_row_lights = list()
    for val in range(21, 25):
        cell = Cell.get_cell_by_value(val)
        if cell.light == True:
            last_row_lights.append(cell.value)
    
    print(last_row_lights)
    if last_row_lights == None:
        return result
    else:
        # For each ON light in the last row, click the cell two rows above it
        # This creates a pattern that can be resolved
        for val in last_row_lights:
            cell = Cell.get_cell_by_value(val - 10)
            print(f'The cell {cell} will be clicked')
            cell.click()
            result.append(cell.value)
            Cell.print_grid()
            
    # This created a pattern that can be resolved by clicking specific cells in the row just above the last row
    for val in last_row_lights:
        cell = Cell.get_cell_by_value(val - 5)
        print(f'The cell {cell} will be clicked')
        cell.click()
        result.append(cell.value)
        Cell.print_grid()
