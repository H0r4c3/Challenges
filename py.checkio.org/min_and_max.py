...
The optional keyword-only key argument specifies a function of one argument that is used to extract a comparison key from each 
list element (for example, key=str.lower).
...
def min(*args, **kwargs):
    return sorted(iter(args[0]) if len(args) == 1 else args, key = kwargs.get("key", None))[0]

def max(*args, **kwargs):
    return sorted(iter(args[0]) if len(args) == 1 else args, key = kwargs.get("key", None), reverse = 1)[0]
